...
dmr_identification<-function(x,diff_meth_cluster,data,legacy_data)
{
  ...
  cpg_names<-as.vector(data[,"CHR"])
  start_cpg<-sapply(df_unique2$start_point,function(x){cpg_names[x] })
  end_cpg<-sapply(df_unique2$end_point,function(x){cpg_names[x] })
  df_unique$start_point<-start_cpg
  df_unique$end_point<-end_cpg

  CHR<-sapply(df_unique$start_point,function(x)
    {legacy_data[legacy_data$IlmnID==x,"CHR"]})
  map_start<-sapply(df_unique$start_point,function(x)
    {legacy_data[legacy_data$IlmnID==x,"MAPINFO"]})
  map_end<-sapply(df_unique$end_point,function(x)
    {legacy_data[legacy_data$IlmnID==x,"MAPINFO"]})
  df_unique$CHR<-CHR
